...
- 11/5凍結の受入条件: `bundle exec rspec spec/demo/` green（デモクエリ全件の期待件数一致。`docs/demo/aql-queries.md`・`docs/design/demo-queries-plan.md` 5節）。**シード経路とQ1相当のフォーム経路統合specで検証済み。フォーム保存経路のAQL合流は`skoba/anlage#10`で解消済み**

...
## 将来課題（2026-08-25、人間レビュー完了時点で追記）

- Phase 2起動時、評価v2を臨床レビュー込みで設計する（期待集合オプション——「血圧」
  「血圧測定」のような複数正解archetypeを許す集合ベース評価への移行、負例セット
  ——bigram偽陽性の系統的収集、保留問2件の出題化——`既往歴→story`・
  `検体→CLUSTER.specimen`。いずれも`spec/fixtures/pathcards_eval_seed.yml`の
  該当問注記に詳細あり）。

**語彙レジスター観察（WP4人間レビューでの横断発見、WP5/voice_aliases設計への入力）**:
記録語彙（カルテ記載語）と説明語彙（患者説明・翻訳語）は別レジスターであり
安易な同義語対応は誤り（例:「体格指数」はBMIの説明語彙だが記録語彙では不使用、
q17で判明）。節名（節見出し語）とモデリング語彙（ELEMENT/archetype指示語）も別軸
——一字差ペア「既往」/「既往歴」・「検体」/「検体検査」がその実例で、
「表層の包含≠指示対象の包含」（既往歴の約8割はOBSERVATION.story系・約2割は
problem_diagnosis系、q18注記に分布定量あり）。voice_aliases設計時はこの2軸
（記録語彙/説明語彙、節名/モデリング語彙）を独立変数として扱う必要がある。

## 完了記録（WP4完全クローズ、2026-08-26）

`skoba/anlage#14`のAcceptance criteriaすべて充足。人間レビュー完了日:
2026-08-24（初回17問体制）〜2026-08-26（q06記入漏れの委任による完遂）。

**最終数値（17問体制、`docs/reports/wp4-eval-log.md` `2026-08-25T00:17:09Z`）**:

- Top-1精度: 14/17 (82.35%)
- Top-3精度: 15/17 (88.24%)
- 完全失敗率: 2/17 (11.76%)
- MRR: 0.8529
- intent_tag別内訳（hit=経路成立、top-3以内到達。top-1精度とは別軸——
  詳細は`docs/reports/wp4-log.md` R3）: bigram成立想定 11/11・複合語 4/4・
  同義語ギャップ 0/2

詳細ログ: `docs/reports/wp4-log.md`（R1〜R3）。

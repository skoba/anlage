...
- 真の同義語・訳語ギャップ: 3/18（例:「BMI」対英語ラベル(Body mass index)、「既往」——形態素解析でも救えない、embedding/同義語辞書が必要。当初の例示「体格指数」はWP4人間レビュー（`skoba/anlage#14`）で記録語彙レジスターに不在（カルテ記載語彙ではなく患者説明・翻訳文脈限定）と判明したため訂正、2026-08-25）
...
**Phase 1で解決しない既知の制約**（正直に開示、デモの質疑応答対応と同じ姿勢）: 「BMI」（対 英語ラベル Body mass index）「既往」のような真の同義語・訳語ギャップは、bigram一致では原理的に救えない（字面上の手がかりが無いため）。ハンドロールの同義語辞書での回避は、LLM分業原理（CLAUDE.md規律7: 意味・構造・コード・制約は常にOPT・CKM・用語マスターから実行時に引く）に反する（捏造した意味対応をコードに埋め込むことになる）ため採らない。この制約はPhase 2（embedding）で解決する対象として明示し、デモ台本側では既知の限界として扱う。
...
- 「BMI」（対 英語ラベル Body mass index）「既往」のような同義語・訳語ギャップの解消
...
(ii) 真の同義語ギャップ（BMI↔英語ラベル Body mass index 等）はPhase 2の存在理由として明記する（下記）。

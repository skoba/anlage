...
- **未翻訳3変種の由来（背景、2026-08-26人間報告）**: 上記で確認した3変種
  ——(a) `*text(en)`（マーカー＋原語コード）、(b) `*text`（マーカーのみ、
  原語コード無し）、(c) マーカー無しの原語残存（ProblemListの例）——は
  ランダムな表記ゆれではなく、**OPTエクスポートに使われたツールチェーン
  世代に対応する観測**と解釈できる（Better ADはテンプレートja宣言と
  アーキタイプ側の実ラベル言語との矛盾を検証・拒否するため、より新しい
  世代のエクスポートほど変種が収束する）。詳細は5節の解釈枠を参照。
...
**未翻訳定量の解釈枠（2026-08-26人間報告）**: テンプレートja宣言×
アーキタイプ側en実ラベルの混在は、Ocean Template Designer世代が
言語矛盾を検証しなかったことによる正規のエクスポート形態である
（Better ADは同矛盾を拒否する）。したがって本チェックリスト・
`untranslated_suspect`が観測する未翻訳ノードは、ツールチェーン世代の
指紋であり、資産（アーキタイプ・OPT）そのものの欠陥ではない——検収・
翻訳補完の要否判断はこの前提で行う。

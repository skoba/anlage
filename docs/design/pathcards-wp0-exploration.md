...
**本レポートはWP0（探索）の成果物であり、コード変更は一切行っていない。**

**ステータス: 承認済み（2026-08-22）。** 上記5節の4件の承認事項はいずれも
2026-08-22付で人間の回答・承認を得て決着した（各項目末尾の「→ 決定」参照）。
WP0はこれをもってクローズする。後続WP1（`docs/design/pathcards-schema-v1.md`）
はWP0とは別に、スキーマv1本体そのものの承認（同文書4節 項目1）を別途必要とする。

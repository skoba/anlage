...
## 8. `mml_referral.opt` 診断ドロップ結果（2026-08-26、見送りを推奨）

`skoba/mml`（working directory外、本項の指示に基づき読み取りのみ実施）
の`mml_referral.opt`（905KB・42埋め込みルート・Ocean TD系、実測で
裏付け済み）を診断した。詳細: `docs/reports/referral-intake-log.md`。

**要点**: 136カード中、日本語文字種を含むlabel/descriptionは**0件**
（ADLソースレベルでも依存16アーキタイプ全てにja翻訳無し——本項7項
「38/522」前提値は本テンプレートの依存クロージャには当てはまらない、
実測0/16）。「既往歴」「紹介」等の基本語彙を含む全試験クエリが
**0ヒット**。取り込み3分岐（そのまま／ADパッチ後／見送り）のうち
**見送りを推奨**（v1範囲確定済みの合意と整合・投資対効果の低さ・
